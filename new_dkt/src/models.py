...
class LastQuery(nn.Module):
    def __init__(self, config):
        ...
        self.Q1 = nn.Sequential(
            nn.Linear(config.attention_size, (config.attention_size // 3) * 2, bias=False),
            nn.Dropout(config.drop_out),
        )
    ...
    def forward(self, cate_x, cont_x, mask, answers):
        ...
        Zs = []
        for i in range(self.config.num_heads):
            Q = self.attentions[i][f'Q{i}'](x[-1:, :, :])
            K = self.attentions[i][f'K{i}'](x)
            V = self.attentions[i][f'V{i}'](x)
            score = torch.matmul(Q, K.transpose(-2, -1))
            score = torch.div(score, self.config.attention_size ** 0.5)
            score = torch.softmax(score, dim=-1)
            Z = torch.matmul(score, V)
            Zs.append(Z)
        ...
        # score = torch.matmul(q1, k1.transpose(1, 2))
        # score = torch.div(score, self.config.attention_size ** 0.5)
        # score = torch.softmax(score, dim=-1)
        # z1 = torch.matmul(score, v1)

        # q2 = self.Q2(x[-1:, :, :])
        # k2 = self.K2(x)
        # v2 = self.V2(x)

        # score = torch.matmul(q2, k2.transpose(1, 2))
        # score = torch.div(score, self.config.attention_size ** 0.5)
        # score = torch.softmax(score, dim=-1)
        # z2 = torch.matmul(score, v2)
        ...
        out = self.fc(hs[-1:, :, :])
        out = out.transpose(0, 1) # B, S 로 변경
        return out.squeeze(-1) 
